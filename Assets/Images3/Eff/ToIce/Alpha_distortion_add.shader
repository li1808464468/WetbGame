...
Shader "UI_Shader/Alpha/distort_add" {
	Properties {
		...
		SubShader {
      	...
      	float _MainPannerY;
			    ...
			    resUV.y += fmod(_MainPannerY*_Time.y,1);
					...
					fixed4 _MainAlpha_var = tex2D(_MainAlpha,resUV);

					half2 maskUV = i.uvMask;
					fixed4 _MaskTex_var = tex2D(_MaskTex,maskUV);

          fixed3 emissive = (_MainColor.rgb*_Brightness*pow(_MainTex_var.rgb,_Contrast)*i.vertexColor.rgb)*(_MainColor.a*_MainAlpha_var.r*i.vertexColor.a*_MaskTex_var.r);
